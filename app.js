...
async function loadVocab() {
  try { const resp = await fetch('vocab_map.json'); VOCAB = await resp.json(); }
...
function getScopeForGrade(grade){ return SCOPE[String(grade).trim()] || []; }

function safePopulateScenarios(){
  const gradeInput = document.getElementById('grade');
  const scenarioSel = document.getElementById('scenario');
  const themeSel = document.getElementById('theme');
  scenarioSel.innerHTML = ''; themeSel.innerHTML = '';
  const grade = (gradeInput.value || '').toString().trim();
  const list = getScopeForGrade(grade);
  if (!list.length){
    // Si no hay escenarios (p. ej. campo vacío), usa un valor por defecto y avisa
    console.warn('No se encontraron escenarios para grade=', grade, '. Asignando valor por defecto 1.');
    gradeInput.value = grade || '1';
  }
  const finalList = getScopeForGrade(gradeInput.value);
  finalList.forEach(item => {
    const opt = document.createElement('option');
    opt.value = item.scenario; opt.textContent = item.scenario;
    scenarioSel.appendChild(opt);
  });
  safePopulateThemes();
}

function safePopulateThemes(){
  const grade = (document.getElementById('grade').value || '').toString().trim();
  const scenario = document.getElementById('scenario').value;
  const themeSel = document.getElementById('theme');
  themeSel.innerHTML = '';
  const list = getScopeForGrade(grade);
  const found = list.find(i => i.scenario === scenario);
  (found?.themes || []).forEach(t => {
...
window.addEventListener('DOMContentLoaded', async () => {
  await loadVocab();
  // Si el campo grade está vacío, inicializa con 1
  const gradeInput = document.getElementById('grade');
  if(!gradeInput.value) gradeInput.value = '1';
  safePopulateScenarios();
  gradeInput.addEventListener('input', safePopulateScenarios);
  document.getElementById('scenario').addEventListener('change', safePopulateThemes);
});
